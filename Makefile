...
SRCS        := $(SRCDIR)m2m/M2MSystem.c \
               $(SRCDIR)m2m/io/M2MHeap.c \
               $(SRCDIR)m2m/io/M2MByteOrder.c \
               $(SRCDIR)m2m/lang/M2MString.c \
               $(SRCDIR)m2m/time/M2MDate.c \
               $(SRCDIR)m2m/log/M2MLogLevel.c \
               $(SRCDIR)m2m/log/M2MLogger.c \
               $(SRCDIR)m2m/util/M2MBase64.c \
               $(SRCDIR)m2m/util/list/M2MList.c \
               $(SRCDIR)m2m/security/acl/M2MACLPermission.c \
               $(SRCDIR)m2m/security/acl/M2MACLUser.c \
               $(SRCDIR)m2m/security/acl/M2MACLGroup.c \
               $(SRCDIR)m2m/security/acl/M2MACL.c \
               $(SRCDIR)m2m/security/M2MCRC32.c \
               $(SRCDIR)m2m/security/M2MSHA1.c \
               $(SRCDIR)m2m/net/network/M2MIPAddress.c \
               $(SRCDIR)m2m/io/M2MFileDescriptor.c \
               $(SRCDIR)m2m/io/M2MDirectory.c \
               $(SRCDIR)m2m/io/M2MFile.c \
               $(SRCDIR)m2m/log/M2MFileAppender.c \
               $(SRCDIR)m2m/db/M2MSQLiteDataType.c \
               $(SRCDIR)m2m/db/M2MSQLite.c \
